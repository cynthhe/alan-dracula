...
create or replace view prod_games.reporting.arcade_7plusdaysretainedgamesplayed as
select
...
join (select 
      ...
      from(select 
           ...
           group by 1)
      where date_diff >= 7 and sessions >= 5) b on b.userid = a.userid
      group by 1,2;

-- Looker permissions for reporting view
grant select on prod_games.reporting.arcade_7plusdaysretainedgamesplayed to looker_read;
