...
SELECT DISTINCT
    ...
    ,(COUNT(DISTINCT userid)/ (SELECT COUNT(DISTINCT userid) 
                                     ...
                                     WHERE country LIKE 'US' AND userid IN (SELECT userid
                                                                            FROM prod_games.arcade.first_played_date
                                                                            WHERE start_date >= '3/4/2019')
                                     AND game_name = 'Cut It Out (FULL)' -- Specify game name here
                              )
     ) AS percent_of_users
FROM prod_games.arcade.achievement
WHERE game_name = 'Cut It Out (FULL)' -- Specify game name here
AND achievement_name IS NOT NULL
GROUP BY 1,2
UNION
SELECT DISTINCT
...
SELECT DISTINCT
    ...
    ,(COUNT(DISTINCT userid)/ (SELECT COUNT(DISTINCT userid) 
                                     ...
                                     WHERE country LIKE 'US' AND userid IN (SELECT userid
                                                                            FROM prod_games.arcade.first_played_date
                                                                            WHERE start_date >= '3/4/2019')
                                     AND game_name = 'Ready to Roar' -- Specify game name here
                              )
     ) AS percent_of_users
FROM prod_games.arcade.achievement
WHERE game_name = 'Ready to Roar' -- Specify game name here
AND achievement_name IS NOT NULL
GROUP BY 1,2
UNION
SELECT DISTINCT
